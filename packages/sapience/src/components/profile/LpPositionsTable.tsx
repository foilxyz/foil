        ...
        <Table>
          ...
          <TableBody>
            {validPositions.map((position: PositionType) => {
              const { market_group } = position.market || {};
              const baseUnit = `${market_group?.baseTokenName || 'Base'}`;
              const quoteUnit = `${market_group?.collateralSymbol || 'Quote'}`;
              const priceUnit = `${market_group?.collateralSymbol || 'Quote'}/${market_group?.baseTokenName || 'Base'}`;

              const isClosed =
                position.lpBaseToken === '0' && position.lpQuoteToken === '0';

              const chainShortName = market_group?.chainId
                ? getChainShortName(market_group.chainId)
                : 'unknown';
              const positionUrl = `/positions/${chainShortName}:${market_group?.address}/${position.market?.marketId}?positionId=${position.positionId}`;
              ...
              const marketAddress = market_group?.address || '';
              const chainId = market_group?.chainId || 0;
